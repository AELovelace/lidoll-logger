...
2. Copy `config.example.h` to **`config.h`** in this folder. Set the 2.4 GHz Wi-Fi SSID/password, `STATS_API_BASE`, and `STATS_TOKEN`. Leave the API base's trailing slash. The current LAN example is `http://10.1.1.23:4173/tracker/api/statistics/v1/`; change it if the tracker is reached at another address.
3. For HTTPS, also fill `TLS_ROOT_CA` with the trusted root CA PEM for the tracker. Keep NTP reachable so the device can validate certificate dates. HTTP works on a trusted LAN; it sends the credential unencrypted. No redirect following or insecure TLS fallback is enabled.
...
- **Everyone / Individual** switches between combined totals and the drilldown.
- **Prev / Next** selects a participant and enters individual view. It pages the server's directory, supporting more people than can fit in device RAM.
...
Networking runs in a FreeRTOS task; the main task owns graphics and touch. Selection serials discard late responses, queues retain the newest requested selection, response allocations are limited to 32 KiB, and connect/read timeouts prevent a stalled connection from blocking the UI. On transient failures, previously loaded values are marked **STALE**; an observed 401/403 clears them. Nothing is written to SD or persistent storage, and no statistics are sent to llama.cpp or Discord by this device. A powered offline device can retain its last displayed values until restarted or refreshed.
...
2. Valid admin token loads both everyone and a named individual; counts match the admin records for the same dates.
3. Rapid participant/window switching never shows values under the wrong person's name.
4. Disable Wi-Fi or stop the server: touch remains responsive, old values say STALE, and reconnection recovers.
5. Revoke the token or demote its issuing administrator: the next API response clears statistics and reports access denied.
6. Let the panel run across midnight LA time and under sustained Wi-Fi traffic; check date rollover and RGB stability. Use the power supply recommended by Elecrow.
